...
- **`config/`** — YAML config (`-c` flag). CLI flags override YAML only when explicitly set (`flag.Visit`). Auth and stats are configured here; helpers `UseHTTPAuth()`, `StatsEnabled()`, `AuthCacheTTL()`, `AuthNegativeCacheTTL()` gate the wiring in `main.go`.
