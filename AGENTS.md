...
- **`auth/`** — pluggable `Authenticator` interface: `Authenticate(addr, authBlob, tx) -> (id, ok, err)`. Implementations: `password` (default, constant single password), `http` (POSTs to a hysteria-2-compatible external backend), and `cache` (a TTL+LRU wrapper over two independent caches — positive results `authBlob->id` (default TTL 10s) and negative results (rejected `authBlob`s, default TTL 60s) — so a stale/revoked client that keeps reconnecting stops hammering the backend; **backend errors always fall through and are never cached**, and separate caches keep a flood of bad blobs from evicting good entries). The returned `id` is the stable key for stats. Selected in `cmd/server/main.go` from config.
- **`stats/`** — `Registry` tracks per-id traffic + live sessions. The session reports bytes through the `TrafficCounter` interface (`Session.Identity`), declared in the session package to avoid an import cycle; `Kickable` plays the same role for the stats→session direction. `http.go` exposes a hysteria-2-compatible API (`/traffic`, `/traffic?clear=true`, `/online`, `/kick`, `/dump/streams`) guarded by a shared `Secret`. There is **no background sweeper** — `/traffic`-tracked memory is reclaimed only by `?clear=true` (offline entries deleted, live ones zeroed in place). The `/dump/streams` `connection` is a **per-device logical id** (`stats.Conn`, inferred from `auth`+source IP since anytls has no on-wire device identity), reference-counted via `AcquireConn`/`ReleaseConn` and freed when the device's last session drops — so one device's pooled TLS sessions share one `connection` and its streams get monotonic ids, mirroring a hysteria QUIC connection. `/online` likewise counts devices (source-IP-deduped), not raw sessions.
- **`acl/`** — hysteria-2-consistent ACL engine (rule syntax, matching order, LRU result cache), without outbounds control: the only valid rule outbounds are the built-ins `direct`, `reject`, `default` (= `direct`); hijack addresses are supported. Rules compile to an `Action` at startup via `config` (`acl.file` or `acl.inline`, mutually exclusive). `acl/v2geo/` loads v2ray `geoip.dat`/`geosite.dat` for `geoip:`/`geosite:` rules; `FileGeoLoader` lazy-loads them and auto-downloads from the Loyalsoldier mirror (7-day default refresh, overridable via `acl.geoUpdateInterval`). In `cmd/server/acl.go`, domain destinations are resolved **before** matching so IP rules apply to domain requests, and TCP dials the resolved IP (hysteria's resolver-chain behavior). UDP-over-TCP streams get per-packet checks through `aclPacketConn`: rejected packets are dropped silently, decisions cached per destination for the stream's life. Geo-dependent unit tests skip unless the `.dat` fixtures are present under `acl/v2geo/`.
- **`config/`** — YAML config (`-c` flag). CLI flags override YAML only when explicitly set (`flag.Visit`). Auth, ACL and stats are configured here; helpers `UseHTTPAuth()`, `ACLEnabled()`, `StatsEnabled()`, `AuthCacheTTL()`, `AuthNegativeCacheTTL()` gate the wiring in `main.go`.
